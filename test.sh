...
RESPONSE=$(curl http://127.0.0.1:5000/post -X POST -d '{"msg": "hi my name is archa"}')
...
curl http://127.0.0.1:5000/post -X POST -d '{"msg": "hi my name is archa"}'
curl http://127.0.0.1:5000/post -X POST -d '{"msg": "hi my name is archa"}'
RESPONSE=$(curl http://127.0.0.1:5000/post/fulltext/"hi%20my%20name%20is%20archa")
counter=0
while [ 1 ]
do
  msg=$(echo $RESPONSE | jq -r '.[0].msg')
  if [ "$msg" != "hi my name is archa" ]; then
